...
Defines the types of blob storage the destination remote file system the file will be uploaded. Currently, the supported blob storage is `GCS` (google cloud storage) and `S3` (amazon cloud storage).
