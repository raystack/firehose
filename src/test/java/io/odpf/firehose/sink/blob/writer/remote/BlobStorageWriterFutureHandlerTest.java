...
public class BlobStorageWriterFutureHandlerTest {
    ...
    public void shouldNotFilterUnfinishedFuture() {
        ...
        BlobStorageWriterFutureHandler handler = new BlobStorageWriterFutureHandler(future, localFileMetadata, instrumentation);
    ...
    public void shouldFilterFinishedFuture() throws Exception {
        ...
        Mockito.verify(instrumentation, Mockito.times(1)).logInfo("Flushed to blob storage {}", "/tmp/test");
        Mockito.verify(instrumentation, Mockito.times(1)).incrementCounter(BlobStorageMetrics.FILE_UPLOAD_TOTAL, Metrics.SUCCESS_TAG);
        Mockito.verify(instrumentation, Mockito.times(1)).captureCount(BlobStorageMetrics.FILE_UPLOAD_BYTES, 1024L);
        Mockito.verify(instrumentation, Mockito.times(1)).captureDuration(BlobStorageMetrics.FILE_UPLOAD_TIME_MILLISECONDS, 1000L);
    }
    ...
    public void shouldThrowException() throws Exception {
        ...
        Mockito.when(localFileMetadata.getFullPath()).thenReturn("/tmp/test");
        Mockito.when(localFileMetadata.getSize()).thenReturn(1024L);
        Assert.assertThrows(BlobStorageFailedException.class, () -> handler.isFinished());
    }
}
