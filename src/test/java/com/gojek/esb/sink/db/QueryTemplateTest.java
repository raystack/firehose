...
public class QueryTemplateTest {
    ...
    @Mock
    private DBSinkConfig dbSinkConfig;

    @Mock
    private ProtoToFieldMapper protoToFieldMapper;

    @Mock
    private EsbMessage mockEsbMessage;

    @Before
    public void setup() {

        when(dbSinkConfig.getTableName()).thenReturn("table");

        Properties properties = new Properties();
        properties.put("1", "order_number");
        properties.put("2", "event_timestamp");
        properties.put("3", "feedback_rating");
        when(dbSinkConfig.getProtoToFieldMapping()).thenReturn(properties);
        ...
        when(dbSinkConfig.getUniqueKeys()).thenReturn(String.join(",", ""));
    }

    private void addUniqueKeys(String uniqueKeys) {

        when(dbSinkConfig.getUniqueKeys()).thenReturn(String.join(",", uniqueKeys));
    }
    ...
    public void shouldUseSingleUniqueKeyColumnForOnConflictResolution() throws IOException, SQLException {
        ...
        QueryTemplate queryTemplate = new QueryTemplate(dbSinkConfig, protoToFieldMapper);
        String actualSql = queryTemplate.toQueryString(esbMessage);
    ...
    public void shouldUseMultipleUniqueKeyColumnForOnConflictResolution() throws IOException, SQLException {
        ...
        QueryTemplate conflictingQueryTemplate = new QueryTemplate(dbSinkConfig, protoToFieldMapper);
        EsbMessage esbMessage = new EsbMessage("key".getBytes(), "msg".getBytes(), "topic1", 0, 100);
        String actualSql = conflictingQueryTemplate.toQueryString(esbMessage);
    ...
    @Test
    public void shouldAddAuditColumnsWhenEnabled() {
        when(dbSinkConfig.getAuditEnabled()).thenReturn(true);

        QueryTemplate queryTemplate = new QueryTemplate(dbSinkConfig, protoToFieldMapper);
        EsbMessage esbMessage = new EsbMessage("key".getBytes(), "msg".getBytes(), "topic1", 0, 100);
        String actualSql = queryTemplate.toQueryString(esbMessage);

        String expectedSql = "INSERT INTO table ( feedback_rating,event_timestamp,order_number,"
                + "kafka_audit_topic,kafka_audit_partition,kafka_audit_offset ) "
                + "values ( '5', 'ts1', 'order_1', 'topic1', '0', '100' ) ";

        Assert.assertEquals(actualSql, expectedSql);
    }

    @Test
    public void shouldAddAuditColumnsOnUpdateWhenEnabled() throws IOException, SQLException {
        addUniqueKeys("order_number, event_timestamp");
        when(dbSinkConfig.getAuditEnabled()).thenReturn(true);
        ...
        String expectedSql = "INSERT INTO table ( feedback_rating,event_timestamp,order_number,"
                ...
                + "DO UPDATE SET ( feedback_rating,kafka_audit_topic,kafka_audit_partition,kafka_audit_offset ) = ('5', 'topic1', '0', '100')";
    ...
    public void shouldAddNestedColumnsToQuery() throws IOException, SQLException {
        ...
        when(dbSinkConfig.getProtoToFieldMapping()).thenReturn(properties2);
    ...
    public void shouldUseKafkaRecordKey() throws Exception {
        when(dbSinkConfig.getKafkaRecordParserMode()).thenReturn("key");
        when(mockEsbMessage.getLogKey()).thenReturn("key".getBytes());
        QueryTemplate queryTemplate = new QueryTemplate(dbSinkConfig, protoToFieldMapper);
        queryTemplate.toQueryString(mockEsbMessage);
        verify(mockEsbMessage, times(1)).getLogKey();
    }

    @Test
    public void shouldUseKafkaRecordMessage() throws Exception {
        when(dbSinkConfig.getKafkaRecordParserMode()).thenReturn("message");
        when(mockEsbMessage.getLogMessage()).thenReturn("message".getBytes());
        QueryTemplate queryTemplate = new QueryTemplate(dbSinkConfig, protoToFieldMapper);
        queryTemplate.toQueryString(mockEsbMessage);
        verify(mockEsbMessage, times(1)).getLogMessage();
    }
}
